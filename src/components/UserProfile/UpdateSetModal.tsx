...
const UpdateSetModal = (props: iUpdateSetModal) => {
  ...
  const updateTagNameAndColor = (e: any , key: number) => {
    ...
    const applyChanges = async function () {
            ...
            try {
                dispatch(loading());
                let setToSave_ : SetDto = _updateSetState.setToSave;
                let setToSaveId_ : string = _updateSetState.id;
                
                let newly_created_set = await updateSet(setToSaveId_, setToSave_);
                dispatch(clearTags());
                setNewSet('');
                history.push('/study')
